...
app._ingresar()
app.update()
assert app.lbl_login.cget("text") == "El usuario no existe. Verifique el nombre.", \
    app.lbl_login.cget("text")
...
def buscar_canvas(raiz):
    ...
    def caminar(nodo):
        for hijo in nodo.winfo_children():
            if (type(hijo).__name__ == "Canvas"
                    and type(hijo.master).__name__ == "CTkFrame"
                    and type(hijo.master.master).__name__ != "CTkScrollableFrame"):
...
clase_plano_seleccionado = gui.ctk.CTkButton(
    app.dashboard_empleado, text="X",
    fg_color=gui.TEMAS["oscuro"]["PRIMARY"], text_color="#FFFFFF",
)
...
assert clase_plano_seleccionado.cget("fg_color") == gui.TEMAS["claro"]["PRIMARY"], \
    "plano seleccionado conserva PRIMARY"
...
app._ingresar()
app.update()
assert hasattr(app, "panel_gestion") and app.panel_gestion is not None, \
    "admin debe entrar a PanelGestion"
...
app.update()
assert app.frame_publico.winfo_ismapped() and hasattr(app, "entrada_usuario"), \
    "volver al login tras gestion"
print("OK volver al login tras gestion")

eventos = db.listar_auditoria(200)
cambios = [e for e in eventos
           if e.get("tabla") == "users" and e.get("accion") == "ACTUALIZAR"
           and "(cambiado)" in str(e.get("valores_nuevos", {}))]
